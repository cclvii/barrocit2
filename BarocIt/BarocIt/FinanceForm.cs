...
namespace BarocIt
{
    public partial class FinanceForm : MainForm
    {
        ...
        {
            // TODO: This line of code loads data into the 'dsPayments.tbl_payments' table. You can move, or remove it, as needed.
            this.tbl_paymentsTableAdapter.Fill(this.dsPayments.tbl_payments);
        ...
        private void ShowDebtor(int clientId)
        {
            ...
            object[] datas = DBHandler.DebtorDatasToArray(clientId);//string[] ?
        ...
        private void ShowProjects(int clientId)
        {
            dg_projects.Show();
            tbl_salesProjectsTableAdapter.FillByClientId(dsFinanceProjects.tbl_salesProjects, clientId);
            
        }

        private void ShowBill(int projectId)
        {
            gb_bill.Show();
        }

        private void ShowPayments(int billId)
        {
            dg_payments.Show();
            tbl_paymentsTableAdapter.FillByInvoiceId1(dsPayments.tbl_payments, billId);
        }
        ...
        private void dg_projects_RowStateChanged(object sender, DataGridViewRowStateChangedEventArgs e)
        {
            if (e.StateChanged == DataGridViewElementStates.Selected) //&& e.Row.Index < 
            {
                ...
                ShowBill(int.Parse(e.Row.Cells[0].Value.ToString().Trim()));
            }
        }
