...
namespace BarrocITApp
{
    class DBHandler
    {
        ...
        {
            con = new SqlConnection(@"Data Source=(LocalDB)\v11.0; AttachDbFilename = C:\Users\admin\Documents\GitHub\Barroc - IT\BarrocITApp\BarrocITApp\BarrocITDB.mdf; Integrated Security = True; Connect Timeout = 30");
        }
            ...
            try
            {
                ...
                while (dataReader.Read())
                {
                    if (dataReader["username"].ToString().Trim() == username && dataReader["password"].ToString().Trim() == password)
                    {
                        ...
                        switch((int)dataReader["username"])
                        {
                            case 0:
